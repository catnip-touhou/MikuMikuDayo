...
- バージョンごとに不具合修正や新要素の追加がありますから、レンダリング結果の見た目は変わる事があります。sdPBRの頃は見た目の互換性に少しは気を払っていましたが、正しい実装をサボる口実にしかならないので、今後は不具合修正や新要素の追加を優先し、バージョン間のレンダリング結果の不一致についてはあまり気にしない事にします。
- 以前のバージョンから引き続き使用する場合は別のフォルダにインストールするのが確実ですが、見た目の一貫性とか拘らないしどうしてもストレージ容量をケチりたいんじゃ！という事で上書きして使う場合は必ず`shadercache`フォルダの内容を消去してください
...
## 大体できました
しかし完成と言えるのはきっとまだ先でしょう。ひきつづきよろしく  
