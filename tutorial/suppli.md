...
MikuMikuDayoではエフェクトの実行される順序は明確に決まっていて1フレームのレンダリングが実行されると以下の順でエフェクトが実行されます

1. デフォーマ
2. レンダラ
3. ポストプロセス

デフォーマとポストプロセスは複数存在する可能性がありますが、それらの間の実行順序は`Models`ウィンドウの`order`ダイアログで指定される通りです。

1つのエフェクト内でのコードの実行順は`passes`配列内で先に定義されるパスから順になります。
