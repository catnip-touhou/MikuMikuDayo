...
| **Audio** | `Wave.ixx` | **Audio Processing**<br>WAVファイルの読み込み、再生、波形データの解析（リップシンクやタイムライン表示用）。 | `class Wave`<br>`GetPCMData()` |
...
| Module Name | Source File | Description & Responsibilities |
| :--- | :--- | :--- |
| **Keyframe Logic** | `keyframeUpdaterDayo.h` | **Keyframe Algorithms**<br>キーフレームの補正（位置・回転のスケール/オフセット）、物理焼き込み (Physics Baking)、反転ペースト、Undo/Redo用データの生成ロジック。 |
